...
// ─── HYBRID ADDITION ───────────────────────────────────────────────
// setup_only: registers bluepad32 HID handlers WITHOUT starting the run loop.
void setup_only(Gamepad(&gamepads)[MAX_GAMEPADS])
{
    for (uint8_t i = 0; i < MAX_GAMEPADS; ++i)
    {
        bt_devices_[i].gamepad = &gamepads[i];
    ...
    uni_platform_set_custom(get_driver());
    uni_init(0, nullptr);   // HYBRID_BT_ALREADY_INITIALIZED: skips l2cap_init + hci_power_control
    ...
    btstack_run_loop_set_timer(&led_timer_, LED_CHECK_TIME_MS);
    btstack_run_loop_add_timer(&led_timer_);
    // run loop started externally after btstack_main (audio)
}
// ───────────────────────────────────────────────────────────────────
