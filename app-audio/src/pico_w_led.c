...
// HIBRIDO: CORRECAO CRITICA. Originalmente este projeto escrevia no
// ULTIMO byte/pagina da flash FISICA inteira (FLASH_SIZE_BYTES - 1).
// Nesta versao hibrida, FLASH_SIZE_BYTES reflete o tamanho FISICO
// total (4MB) -- necessario para o SDK calcular enderecos XIP
// corretamente -- mas o ultimo byte da flash fisica pertence ao
// CONFIG_SECTOR do bootloader (ver shared/flash_map.h), NAO a este
// app! Escrever ali corromperia o sistema de "lembrar o ultimo modo
// que funcionou".
//
// A correcao usa o fim da PARTICAO deste app (HYBRID_APP_FLASH_OFFSET
// + HYBRID_APP_FLASH_LENGTH, definidos via -D no CMakeLists a partir
// de shared/flash_layout.cmake) como referencia, em vez do fim da
// flash fisica inteira.
#if defined(HYBRID_APP_FLASH_OFFSET) && defined(HYBRID_APP_FLASH_LENGTH)
    #define HYBRID_APP_FLASH_END   (HYBRID_APP_FLASH_OFFSET + HYBRID_APP_FLASH_LENGTH)
    #define TARGET_OFFSET          (HYBRID_APP_FLASH_END - 1)
#else
...
// HIBRIDO: mesma correcao aplicada acima para TARGET_OFFSET -- usa o
// fim da PARTICAO deste app, nao o fim da flash fisica inteira.
#if defined(HYBRID_APP_FLASH_OFFSET) && defined(HYBRID_APP_FLASH_LENGTH)
    #define LAST_PAGE_BASE        (HYBRID_APP_FLASH_END - FLASH_PAGE_SIZE)
#else
