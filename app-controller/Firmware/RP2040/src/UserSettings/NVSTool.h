...
private:
    ...
    // CORRECAO HIBRIDA: neste fork, PICO_FLASH_SIZE_BYTES reflete o
    // tamanho FISICO total da flash (4MB), necessario para o SDK
    // calcular enderecos XIP corretamente quando o app roda a partir
    // de um OFFSET (nao do endereco 0). Se o calculo do NVS_START_OFFSET
    // usasse PICO_FLASH_SIZE_BYTES diretamente, ele cairia no FINAL da
    // flash FISICA -- que pertence a particao do OUTRO app (audio) --
    // corrompendo mutuamente os dois firmwares.
    //
    // HYBRID_APP_FLASH_OFFSET e HYBRID_APP_FLASH_LENGTH sao definidos
    // via -D no CMakeLists.txt deste app (shared/flash_layout.cmake),
    // e delimitam o espaco de flash que pertence SOMENTE a este app.
    // O NVS fica alocado nos ultimos NVS_SECTORS setores DENTRO dessa
    // particao, nunca fora dela.
#if defined(HYBRID_APP_FLASH_OFFSET) && defined(HYBRID_APP_FLASH_LENGTH)
    static constexpr uint32_t NVS_START_OFFSET = HYBRID_APP_FLASH_OFFSET + HYBRID_APP_FLASH_LENGTH - FLASH_SECTOR_SIZE * NVS_SECTORS;
#else
